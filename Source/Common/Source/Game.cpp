...
namespace BBB
{
	...
	ZED_UINT32 Game::Initialise( )
	{
		...
		if( m_GameConfiguration.Read( "bangbangbanquet.config" ) != ZED_OK )
		{
			zedTrace( "Failed to open file: ../bangbangbanquet.config\n" );
			return ZED_FAIL;
		}
		...
		{
			zedTrace( "[BBB::Game::Initialise] <ERROR> "
		...
		m_pRenderer->ClearColour( 0.13f, 0.0f, 0.13f );
